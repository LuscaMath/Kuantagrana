        ...
        <div class="mx-auto max-w-7xl space-y-6">
            ...
            <section class="grid gap-4 sm:grid-cols-3">
                <div class="pixel-card">
                    ...
                    <p class="mt-3 text-2xl font-extrabold">{{ $goals->where('status', 'active')->count() }}</p>
                </div>

                <div class="pixel-card">
                    <p class="text-xs font-extrabold uppercase tracking-[0.16em] text-[color:var(--vm-wood)]">Concluídas</p>
                    <p class="mt-3 text-2xl font-extrabold">{{ $goals->where('status', 'completed')->count() }}</p>
                </div>
            ...
            <section class="pixel-card">
                <div>
                    ...
                    <h3 class="mt-2 text-lg">Seus objetivos financeiros</h3>
                </div>
                        ...
                        <article class="border-4 p-4" style="border-color: var(--vm-border); background-color: #fffdf2;">
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                <div>
                                    ...
                                    <h4 class="mt-2 text-lg font-extrabold">{{ $goal->title }}</h4>
                                    @if ($goal->description)
                                        <p class="mt-2 text-sm leading-6">{{ $goal->description }}</p>
                                    @endif
                                </div>
                            ...
                            <div class="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                ...
                                <div class="flex flex-col gap-3 sm:flex-row">
                                    <a href="{{ route('goals.edit', $goal) }}" class="pixel-btn w-full sm:w-auto">
                                        Gerenciar
                                    </a>
